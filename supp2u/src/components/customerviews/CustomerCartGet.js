...
function CustomerCartGet(props) {
  ...
  const placeOrder = () => {
    ...
    Axios.post(
      `${process.env.REACT_APP_BACKEND_URL}customers/${custy}/orders`,
      orderInfoSend
    )
      .then(res => {
  ...
  const deleteitem = item => {
    ...
    Axios.put(
      `${process.env.REACT_APP_BACKEND_URL}carts/${cart.id}/removeitem`,
      pompom
    )
      .then(res => {
  ...
  useEffect(() => {
    const getList = data => {
      ...
      Axios.get(
        `${process.env.REACT_APP_BACKEND_URL}carts/${data.id}/itemfetch`
      )
        .then(res => {
    ...
    <div className="centerOrder">
      {cart === undefined ? (
        ...
        <div>
          ...
          <button className="buttony" onClick={gobackprev}>
            Go Back to Menu / Add More Items
          </button>
          <DeleteCartButton
            cus_id={custy}
            cart={cart}
            bis_id={cart.business_id}
          />
        </div>
      )}
